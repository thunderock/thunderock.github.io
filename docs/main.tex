...
\section{Work Experience}
  ...
  \resumeSubheading
    ...
    \resumeItemListStart
      \resumeItem{Leading parts of the org's MLOps platform for building, packaging, deploying, and monitoring GPU-accelerated data pipelines and model inference on Kubernetes---spanning the full ML lifecycle from data ingestion and dataset management through inference and data provenance.}
      ...
      \resumeTopic{Foundation Model Training Framework}{Leading development of a PyTorch-based training framework adopted across the org for training generative models on billions of images and videos.}
        ...
        \resumeItemListEnd
      \resumeTopic{Build \& Deployment System}{Built a CLI-driven build system that packages plugin code into Docker contexts, builds images with BuildKit on KEDA-scaled Kubernetes jobs, and pushes to ECR; links built environments to autoscaling inference deployments.}
      \resumeTopic{Data Lineage \& Provenance}{Modeled a provenance graph over derived-from relationships across datasets, assets, builds, and inferences, backed by a Rust (Axum + Diesel + PostgreSQL) control plane serving as the authoritative platform state.}
      ...
      \resumeTopic{MCP Interface for AI Agents}{Exposed the platform API as Model Context Protocol (MCP) tools so AI agents can drive builds, deployments, and inference programmatically.}
    \resumeItemListEnd
